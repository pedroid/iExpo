...
imgPlayer.src = "assets/protagonist.png";
...
function OnResize() {
    ...
    div.style.width = canvas.scrollWidth * 0.9 + "px";
    div = document.getElementById('page');
    div.style.width = canvas.scrollWidth * 0.8 + "px";
}
...
function LoadNPC(mapid) {
    const appNPC = "https://script.google.com/macros/s/AKfycbwd3X18ktS5VJNqPlPic-f_dCqrXs2C03bsvBx8aiODpb9KMEqF/exec";
...
function DrawNpc(npc) {
    ...
    //人物
    else if (npc.type == 'dynamic') {
        DrawPlayer(npc);
    }
}

function ChgRoleDirX(role, destx) {
    if (destx > role.srcx) role.dir = 2;
    else role.dir = 1;
    role.srcx = destx;
}

function ChgRoleDirY(role, desty) {
    if (desty > role.srcy) role.dir = 0;
    else role.dir = 3;
    role.srcy = desty;
}
    ...
    if (role.type == 'dynamic') {
        switch (role.path) {
            ...
            case 'cruise': //兩點來回走動NPC
                ...
                if (Math.abs(ox) > speed) {
                    ChgRoleDirX(role, role.srcx + Math.sign(ox) * speed);
                } else if (Math.abs(oy) > speed) {
                    ChgRoleDirY(role, role.srcy + Math.sign(oy) * speed);
                } else {
...
function DialogCmd() {
    ...
    if (m_curDialog >= NpcRole[m_pickRole].dialog.type.length) {
        m_screen = SCREEN_GAME;
        ShowDialog(false);
        ShowPage(false);
        return;
    }
...
function MapScroll() {
    ...
    var speed = 20; //9正常
    if (Math.abs(ox) > speed) {
        Player.srcx += speed * Math.sign(ox);
        ChgRoleDirX(Player, Player.srcx + Math.sign(ox) * speed);
    } else if (Math.abs(oy) > speed) {
        Player.srcy += speed * Math.sign(oy);
        ChgRoleDirY(Player, Player.srcy + Math.sign(oy) * speed);
    }
...
function onMouseDown(e) {
    ...
    if (m_screen == SCREEN_DIALOG) {
        DialogClick();
        return;
    }
    if (m_screen == SCREEN_PAGE) {
        DialogClick();
        return;
    }
    if (m_screen == SCREEN_GAME) {
        m_pickRole = PickNPC();
        if (m_pickRole >= 0) {
            console.log("pick=" + m_pickRole);
            console.log(NpcRole[m_pickRole].dialog);

            if (NpcRole[m_pickRole].dialog.type.length > 0) {
                m_curDialog = 0;
                DialogCmd();
            }
            return;
...
function onMouseUp(e) {
    ...
    switch (m_screen) {
        case SCREEN_LOAD:
            if (MouseInRect(795, 703, 1161, 797)) {}
            break;
    }
}
...
function game_Render() {
    ...
    switch (m_screen) {
        ...
        case SCREEN_LOAD:
            DrawLoading();
            if (g_loading_ok) m_screen = SCREEN_GAME;
            break;
...
function Interval_Loading() {
    ...
    if (true) {
        ...
        gameIntervalID = setInterval(game_Render, cUpdateTime);
    }
}
